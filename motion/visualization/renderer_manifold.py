...
class PlayerWindow():
	...
	def show(self, window):
		...
		self.loop = window.show_checkbox("Loop", self.loop)
		...
		#Frame / Speed / Data idx
		changed, self.frame_idx = window.show_slider_int("Frame", self.frame_idx, 0, self.max_frame_idx)
		if changed:
...
#--------------------------------
window = gx.Window()
window.init_glfw("Latent Space", 1920, 1152)
window.init_glad()
window.init_imgui()
...
#Cameras
#--------------------------------
camera_fix = gx.Camera(gx.vec3(0.0, 36.0, 40.0), gx.vec3(0, 1, 0), -90.0, -30.0, 1.0)
...
shader_pbr2.set_uniform("albedo", gx.vec3(0.0, 0.0, 1.0))
shader_pbr2.set_uniform("ao", 1.0)
shader_pbr2.set_uniform("metallic", 0.5)
...
player_window = PlayerWindow("Player (Expert)")
...
#---------------------------
# Reload data for player
#---------------------------
def reload_player(motion, motion_recon, z_mb):
	global frame_progress

	pose.apply_amass(motion[0])
	pose.contact_ground(0.0)
	pose_recon.apply_amass(motion_recon[0])
	pose_recon.contact_ground(0.0)
	z_points = []

	for i in range(len(z_mb)):
		z_points.append(gx.vec3(z_mb[i, 0], z_mb[i, 1], 0.0))

	plot.set_points(z_points)
	obj_highlight.position = plot_scale * plot.get_points()[0]
	player_window.frame_idx = 0
	player_window.max_frame_idx = len(motion) - 1
	player_window.load = False
	frame_progress = 0.0
...
#---------------------------
def render_player(motion, motion_recon):
	global frame_progress

	if player_window.change_frame:
		frame_progress = float(player_window.frame_idx)
		pose.apply_amass(motion[player_window.frame_idx])
		pose.contact_ground(0.0)
		pose_recon.apply_amass(motion_recon[player_window.frame_idx])
		pose_recon.contact_ground(0.0)
		obj_highlight.position = plot_scale * plot.get_points()[player_window.frame_idx]
		player_window.change_frame = False

	if player_window.is_playing:
		prev_frame_progress = int(frame_progress)
		frame_progress += window.get_delta_time() * 120.0 * player_window.speed

		if frame_progress > player_window.max_frame_idx:
			if player_window.loop:
				frame_progress = 0.0
			else:
				frame_progress = float(player_window.max_frame_idx)
				player_window.is_playing = False

		player_window.frame_idx = int(frame_progress)

		if prev_frame_progress != player_window.frame_idx:
			pose.apply_amass(motion[player_window.frame_idx])
			pose.contact_ground(0.0)
			pose_recon.apply_amass(motion_recon[player_window.frame_idx])
			pose_recon.contact_ground(0.0)
...
def render_screen():
	...
	for i in range(10):
		...
		if key_states[i] == True and not window.get_key_down(key_vals[i]):
			player_window.chosen[i] = not player_window.chosen[i]
			key_states[i] = False
