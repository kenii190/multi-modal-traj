...
def main():
	...
	for it in range(start_it, n_iter+1):
		#Run the environment
		with torch.no_grad():
			mb_obs, mb_actions, mb_cs, mb_old_a_logps, mb_values, mb_returns = runner.run(policy_net, value_net, enc_net, dis_net)
		...
		if it % disp_step == 0:
			...
			pg_loss, v_loss, ent, dis_loss, dis_real, dis_fake, enc_loss = agent.get_eval()
			...
			print("enc loss         = {:.6f}".format(enc_loss))
